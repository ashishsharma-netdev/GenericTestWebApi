...
public class AdminUsersController(TestPrepDbContext db) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> Users([FromQuery] string? search, [FromQuery] string? status, CancellationToken ct)
    {
        ...
        var users = await query.OrderByDescending(x => x.CreatedAtUtc).Take(500)
            .Select(x => new { x.Id, x.FullName, x.Email, x.IsActive, x.CreatedAtUtc, roles = x.UserRoles.Select(r => r.Role.Name).OrderBy(r => r).ToList() })
            .ToListAsync(ct);
        return Ok(users);
    }
    ...
    public async Task<IActionResult> User(int id, CancellationToken ct)
    {
        ...
        var subscription = await db.Subscriptions.AsNoTracking().Include(x => x.Plan)
            .Where(x => x.UserId == id).OrderByDescending(x => x.ExpiresAtUtc).FirstOrDefaultAsync(ct);
    ...
    public async Task<IActionResult> SetStatus(int id, UserStatusRequest request, CancellationToken ct)
    {
        ...
        if (user is null) return NotFound(new { message = "User not found." });
        if (!request.IsActive && await db.UserRoles.Include(x => x.Role).AnyAsync(x => x.UserId == id && x.Role.Name == "Admin", ct))
            return BadRequest(new { message = "An administrator account cannot be deactivated from this screen." });
