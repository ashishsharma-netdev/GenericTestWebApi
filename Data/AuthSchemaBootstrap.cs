...
IF OBJECT_ID('dbo.Payments','U') IS NULL CREATE TABLE dbo.Payments (Id int IDENTITY(1,1) NOT NULL PRIMARY KEY, UserId int NOT NULL, SubscriptionId int NULL, Provider nvarchar(30) NOT NULL, ProviderOrderId nvarchar(100) NOT NULL, ProviderPaymentId nvarchar(100) NULL, Amount decimal(10,2) NOT NULL, Currency nvarchar(10) NOT NULL, Status nvarchar(30) NOT NULL, CreatedAtUtc datetime2 NOT NULL DEFAULT SYSUTCDATETIME(), PaidAtUtc datetime2 NULL, CONSTRAINT FK_Payments_Users FOREIGN KEY(UserId) REFERENCES dbo.Users(Id) ON DELETE CASCADE, CONSTRAINT FK_Payments_Subscriptions FOREIGN KEY(SubscriptionId) REFERENCES dbo.Subscriptions(Id) ON DELETE SET NULL);
