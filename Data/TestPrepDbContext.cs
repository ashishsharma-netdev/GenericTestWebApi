...
public class TestPrepDbContext(DbContextOptions<TestPrepDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<UserEntity>(e=>{e.ToTable("Users");e.HasKey(x=>x.Id);e.Property(x=>x.FullName).HasMaxLength(150).IsRequired();e.Property(x=>x.Email).HasMaxLength(256).IsRequired();e.Property(x=>x.PasswordHash).IsRequired();e.HasIndex(x=>x.Email).IsUnique();e.HasIndex(x=>x.GoogleSubject).IsUnique();});
        ...
        modelBuilder.Entity<TestAttemptEntity>(e=>{e.ToTable("TestAttempts");e.HasKey(x=>x.Id);e.Property(x=>x.Score).HasPrecision(10,2);e.Property(x=>x.Percentage).HasPrecision(6,2);e.Property(x=>x.IsSubmitted).HasDefaultValue(false);e.HasIndex(x=>new{x.UserId,x.MockTestId,x.IsSubmitted});e.HasOne(x=>x.User).WithMany(x=>x.TestAttempts).HasForeignKey(x=>x.UserId).OnDelete(DeleteBehavior.Cascade);e.HasOne(x=>x.MockTest).WithMany().HasForeignKey(x=>x.MockTestId).OnDelete(DeleteBehavior.Restrict);});
