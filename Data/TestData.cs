...
public static class TestData
{
    ...
    {
        var categoryInfo = Categories.FirstOrDefault(x => x.Name.Equals(category, StringComparison.OrdinalIgnoreCase));
        if (categoryInfo is null) return [];
        var name = categoryInfo.Name;
        var baseId = categoryInfo.Id * 100;
        return
        [
            new(baseId + 1, name, $"{name} Full Mock Test 01", 200, 200, 60, "Latest Pattern"),
            new(baseId + 2, name, $"{name} Full Mock Test 02", 200, 200, 60, ""),
            new(baseId + 3, name, $"{name} Previous Year Paper (2023)", 200, 200, 60, "PYQ"),
            new(baseId + 4, name, $"{name} Sectional Test - Quant", 50, 50, 30, ""),
            new(baseId + 5, name, $"{name} Sectional Test - Reasoning", 50, 50, 30, ""),
            new(baseId + 6, name, $"{name} Sectional Test - English", 50, 50, 30, "")
        ];
    }

    public static MockTest? FindTest(int testId) => Categories.SelectMany(c => GetTests(c.Name)).FirstOrDefault(x => x.Id == testId);
